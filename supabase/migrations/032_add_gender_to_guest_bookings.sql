-- ================================================
-- Add gender column to guest_bookings table
-- ================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    ADD COLUMN gender TEXT CHECK (gender IN ('female', 'male', 'rather_not_say'));
  END IF;
END $$;

-- Add UPDATE policy for guest_bookings (was missing in original migration)
DROP POLICY IF EXISTS "Match creators can update guest bookings" ON guest_bookings;
CREATE POLICY "Match creators can update guest bookings" ON guest_bookings
  FOR UPDATE USING (
    EXISTS (
...
-- Update get_match_by_id function to include gender in guest_bookings
